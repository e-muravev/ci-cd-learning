...
Автоматическая проверка кода после каждого пуша. Содержит:

- Установку зависимостей (`npm ci`)
- Запуск линтеров и тестов (`npm run lint`, `npm run test`)
- Сборку приложения (`npm run build`)
- Сборку приложения и отправку образа в GitHub Container Registry (ghcr.io)

**Цель:** убедиться, что новый код ничего не сломал.

### **CD (Continuous Deployment/Delivery)**

Автоматическая доставка приложения на сервер.
...
- **Версионирование**: Docker-образы хранятся в реестре (GitHub Container Registry). В любой момент можно откатиться к старому образу:
  ```bash
  docker run ghcr.io/...:old_sha
  ```
...
| Ключ               | Описание                                  | Где лежит                                   |
| ------------------ | ----------------------------------------- | ------------------------------------------- |
| **Приватный ключ** | Никому не показывать. Это "ключ от замка" | У тебя локально и в GitHub Secrets          |
| **Публичный ключ** | Можно показывать всем. Это "замок"        | На сервере в файле `~/.ssh/authorized_keys` |

### **Как настроить (краткая инструкция)**

1. **Создать новую пару ключей для GitHub Actions** (чтобы не путать с личными):
...
1. **Job test**: Устанавливает зависимости и запускает тесты
2. **Job build**: Собирает статику (папка `dist`)
3. **Job docker** (только при пуше, не для PR):
...
1. Устанавливается SSH-ключ из Secrets
2. По SSH подключаемся к серверу и выполняем команды:
   ```bash
...
- ✅ Сломал деплой? → Найди в ghcr.io старый тег (SHA) и запусти его вручную на сервере:
  ```bash
  docker run ... ghcr.io/...:старый_sha
  ```
